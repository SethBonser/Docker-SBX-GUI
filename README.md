...
For an agent picking this project back up: the app is built incrementally, each piece built and verified live against a real `sbx` install before moving on (not from documentation assumptions alone — the CLI's actual JSON shapes, error text, and flag requirements were all confirmed by testing, including things that turned out to *not* work as assumed — see the gotchas above). Done so far: scaffold + packaging, the Dashboard, the Create-sandbox wizard with Kits, structured chat panels for Claude, Codex, Gemini, and docker-agent (each with a live permission-mode/hang-avoidance flag and Clear chat), the embedded Terminal tab, per-sandbox Ports and Policy tabs, the global Policy page (tier switcher + custom rules), MCP server management (register/authorize/load), the Secrets manager, the real Settings screen (daemon controls + diagnostics), the Chat-tab `/mcp` status row and `/login`/`/mcp` aliasing, and the Docker/Anthropic sign-in flows described above.
...
- **Password-manager integration for the Secrets page.** This is about the *input* path, not at-rest storage — stored secrets already go straight into `sbx`'s own OS-keychain-backed store (see Security measures), never touched by this app once saved. The gap is that getting a value there today means copy-pasting it from wherever it actually lives into a plaintext `<input>`. Likely shape: shell out to an installed password-manager CLI (1Password's `op`, Bitwarden's `bw`) to fetch a secret by reference and pipe it straight into the existing stdin-based `secretSet`/`runWithStdin` plumbing (see above) — the value would never populate the visible form field or pass through renderer state at all, reusing infrastructure already built rather than adding a new one. Not started; added to the backlog on request.
